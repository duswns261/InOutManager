...
- 입고 제품 요약과 신규 등록 화면의 이미지 우선 배치, 촬영·파일 선택 흐름 추가
- 입고·출고·자재 현황 제품명 실시간 부분·초성 검색 및 수량 내림차순 정렬
- 제품 이미지 없음 상태를 공통 박스 아이콘 placeholder로 통일
- CameraX Preview와 실제 촬영 결과의 화각 일치
...
- 기존 테라코타 브랜드 기반 Material 3 semantic color role로 라이트·다크 테마 정비

관련 Issue:

- Issue #29: 제품 이미지 없음 placeholder를 박스 아이콘으로 통일
- Issue #30: CameraX Preview와 촬영 결과 화각 일치
- Issue #31: 입고 제품 요약 및 제품 이미지 선택 UX 개선
- Issue #32: App Bar 기반 재고 화면 전환 및 제품 등록 진입점 개편
- Issue #33: 입고·출고·자재 현황 제품 검색 및 수량순 정렬
- Issue #34: HomeScreen 안내 문구 제거 및 주요 기능 버튼 중앙 배치
- Issue #39: Material 3 semantic color role 기반 라이트·다크 테마 정비

### 목표

Product Imaging으로 핵심 기능이 갖춰진 뒤, 출시 전 마지막으로 핵심 화면의 정보 밀도와 탐색 방식을 정리하고 이미지 입력 경험의 남은 간극을 보완한다. 제품 데이터 모델을 확장하지 않고 기존 화면의 검색·정렬·이미지 선택·탐색 UX를 개선하며, 확정된 UI를 기존 브랜드 기반의 Material 3 semantic color role로 정비해 시스템 라이트·다크 테마에서 일관된 가독성을 제공한다.

### 대표 작업

- 입고 제품 항목의 요약 정보를 확인할 수 있는 뷰/다이얼로그 추가
- 신규 등록에서는 이미지를 입력 필드보다 먼저 중앙 배치하고, 제품 요약에서는 한 줄 제품명 헤더 다음에 큰 이미지를 중앙 배치
- 제품 요약의 제품명이 말줄임될 때 조건부 펼침 UI로 전체 이름 확인
- 제품 이미지 영역의 + 버튼에서 카메라 촬영 또는 Android Photo Picker를 선택해 이미지를 추가·변경하고 기존 이미지를 독립 제거
- 선택한 외부 이미지를 앱 관리 임시 파일로 가져오고 기존 파일 생명주기 적용
- 입고·출고·자재 현황에 공통 검색 컴포넌트 배치
- 제품명 부분 문자열과 한글 초성 검색 지원
- 세 화면의 제품을 수량 내림차순으로 일관되게 정렬
- 제품 이미지 없음·파일 유실 상태를 박스 outline 아이콘 placeholder로 통일
- CameraX Preview와 ImageCapture를 같은 ViewPort로 묶어 촬영 화각 일치
...
- Issue #29~#34로 확정된 UI를 대상으로 완전한 `LightColorScheme`과 `DarkColorScheme` 정의
- 하드코딩된 라이트 전용 배경·표면·텍스트 색상을 짝이 맞는 Material 3 semantic color role로 치환
- App Bar, 화면 전환 메뉴, 제품 카드와 등록·요약·출고·삭제 dialog의 라이트·다크 대비 통일
- 일반 텍스트 4.5:1, 큰 텍스트와 의미 있는 아이콘·상태 표시 3:1 대비 목표

### 완료 기준

- 제품 요약 보기가 목록의 모든 항목에서 동작한다.
- 신규 등록에서는 이미지가 입력 필드보다 먼저 중앙에 표시되고, 제품 요약에서는 제품명 다음으로 이미지가 위치·수량보다 먼저 크게 중앙에 표시된다.
- 제품 요약의 제품명이 한 줄에서 말줄임될 때만 펼침 진입점이 나타나고, 펼치면 전체 이름을 확인할 수 있다.
- 이미지 유무와 관계없이 제품 이미지 영역의 + 버튼에서 촬영 또는 파일 선택으로 이미지를 추가·변경하고 기존 이미지를 독립 제거할 수 있다.
- 파일 선택에 광범위한 미디어·저장소 권한을 요구하지 않고 앱 관리 저장소에 안전하게 복사된다.
- 세 화면에서 제품명 부분 문자열과 한글 초성으로 실시간 검색할 수 있다.
- 검색 중 일치하지 않는 제품이 숨겨지고 뒤로가기로 전체 목록이 복원된다.
- 세 화면의 제품이 수량 내림차순으로 일관되게 표시된다.
- 이미지 없음·파일 유실 시 제품명의 첫 글자 대신 공통 박스 아이콘이 표시된다.
- CameraX Preview에서 보이지 않은 영역이 촬영 결과에 추가되지 않는다.
...
- 기존 테라코타 브랜드 기반의 라이트·다크 `ColorScheme`이 정의되고 핵심 화면과 modal UI가 짝이 맞는 Material 3 semantic color role을 사용한다.
- 시스템 라이트·다크 모드에서 일반 텍스트와 의미 있는 아이콘·선택 상태가 충분한 대비로 표시되고 모든 핵심 흐름을 조작할 수 있다.
- 기존 입고·출고·재고 조회 흐름이 회귀 없이 동작한다.

### 분리 원칙

```text
Product/Room schema와 재고 비즈니스 규칙 변경은 이 Milestone에 포함하지 않는다.
Domain/Data 변경은 외부 이미지를 앱 관리 파일로 가져오고 기존 제품에 연결하는
파일 생명주기 경계에 필요한 최소 범위만 허용한다.
검색은 로컬 표시 목록에서 수행하며 Room FTS나 서버 검색을 도입하지 않는다.
Issue #39는 #29~#34에서 확정된 UI 전체를 대상으로 하므로 #34 이후에 진행한다.
로고·앱 아이콘·타이포그래피·shape를 포함한 브랜드 전면 재설계와 Material You
dynamic color 기본 활성화는 포함하지 않는다.
```
