...
- Milestone 범위를 벗어난 변경이 포함되지 않는다.
- 불필요한 파일 변경이 없다.
...
- Hilt 적용 후 기존 상품 조회, 추가, 수량 감소, 삭제 기능이 유지된다.
...
- 화면 route 또는 destination이 명확하게 정의되어 있다.
- Home, Inbound, Outbound, Status 화면의 이동 흐름이 구분되어 있다.
- Navigation 적용 과정에서 ViewModel 상태 관리 구조가 불필요하게 변경되지 않았다.
- 화면 이동과 비즈니스 로직이 섞이지 않는다.
- 뒤로가기 동작이 의도한 사용자 흐름과 맞는다.
- Navigation 적용 후 기존 기능이 유지된다.
...
- Hilt, Navigation, HomeScreen 관련 Issue에서는 의도치 않은 schema 변경이 없어야 한다.
...
- 아키텍처 규칙이 바뀌었다면 `docs/architecture-rules.md`를 갱신했다.
- 후속 Issue 흐름이 바뀌었다면 `docs/project-roadmap.md`를 갱신했다.
- Issue 작성 기준이 바뀌었다면 `docs/issue-workflow.md`를 갱신했다.
- 공통 완료 기준이 바뀌었다면 `docs/definition-of-done.md`를 갱신했다.
- Milestone 구성이 바뀌었다면 `docs/project-roadmap.md`를 갱신했다.

---

## 10. Milestone 2 완료 체크리스트

Milestone 2: DI & Navigation Foundation이 완료되려면 다음 항목을 확인한다.

- Issue #11 Hilt DI 적용이 완료되어 있다.
- Issue #12 Navigation Compose 적용이 완료되어 있다.
- Issue #13 HomeScreen 추가가 완료되어 있다.
- Hilt 적용 후 ViewModel 생성 오류가 없다.
- Navigation 적용 후 화면 이동 및 뒤로가기 동작이 정상이다.
- HomeScreen에서 주요 기능 화면으로 이동할 수 있다.
...
- `./gradlew :app:build`가 성공한다.
- Issue의 완료 조건을 모두 확인했다.
