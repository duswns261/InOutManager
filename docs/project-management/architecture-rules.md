...
- Flow 수집 로직은 `init` 블록에 인라인으로 작성하지 않고 의도가 드러나는 함수로 분리하는 것을 우선한다. (예: `observeProducts()`)
...
모든 상태가 `UiState`에 들어가야 하는 것은 아니다.
...
- 사용자 입력 변환(`String → Int` 등)이 실패할 수 있는 경우, 실패를 무시하지 않고 사용자에게 전달할 방법을 함께 검토한다.

---

## 7. DI 규칙

Hilt DI가 적용된 상태다 (Issue #11 완료).

- `@HiltAndroidApp`, `@AndroidEntryPoint`, `@HiltViewModel`을 통해 Hilt 진입점이 구성되어 있다.
- 객체 생성 책임은 `DatabaseModule`, `DataModule`, `UseCaseModule`이 담당한다.
- Hilt Module의 책임과 `@Singleton` scope를 명확히 유지한다.
- DI 변경이 목적이 아닌 Issue에서는 Module 구조와 scope를 변경하지 않는다.
...
- AI Agent 단계별 작업 프로토콜과 양식은 `docs/ai/`에서 관리한다.
